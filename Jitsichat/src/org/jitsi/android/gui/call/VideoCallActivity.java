 ...
 * @author Pawel Domas
 */
public class VideoCallActivity extends OSGiActivity implements
		CallPeerRenderer, CallRenderer, CallChangeListener,
		PropertyChangeListener, ZrtpInfoDialog.SasVerificationListener,
		AutoHideController.AutoHideListener {
	/**
	 * The logger
	 */
	private static final Logger logger = Logger
			.getLogger(VideoCallActivity.class);

	/**
	 * Tag name for fragment that handles proximity sensor in order to turn the
	 * screen on and off.
	 */
	...
	/**
	 * Auto-hide controller fragment for call control buttons. It is attached
	 * when remote video covers most part of the screen.
	 */
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.callIdentifier = getIntent().getExtras().getString(
		...
		if (AndroidUtils.hasAPI(11)) {
			sasToastController = new ClickableToastController(toastView,
					toastclickHandler);
		} else {
			sasToastController = new LegacyClickableToastCtrl(toastView,
					toastclickHandler);
		}
		...
		if (savedInstanceState == null) {
			...
			autoHide = (AutoHideController) fragmentManager
					.findFragmentByTag(AUTO_HIDE_TAG);

			volControl = (CallVolumeCtrlFragment) fragmentManager
					.findFragmentByTag(VOLUME_CTRL_TAG);
		}
	}
	...
	private void initAutoAnswer() {
		...
		String xAuto = getIntent().getExtras().getString(
				CallManager.CALL_AUTO_ANSWER);
		if (xAuto.equals("1")) {
	...
	/*
	 * public void startPlaying(CallPeer callPeer, DataSource soundDataSource)
	 * throws OperationFailedException { //assert callPeer instanceof
	 * CallPeerJabberImpl; MediaAwareCallPeer cp = (MediaAwareCallPeer)
	 * callPeer; MediaAwareCallPeer aa; AudioMediaStreamImpl audioMediaStream =
	 * (AudioMediaStreamImpl ) cp.getMediaHandler().getStream(MediaType.AUDIO);
	 * AudioMediaDeviceSession deviceSession =
	 * audioMediaStream.getDeviceSession(); assert deviceSession != null; assert
	 * deviceSession.getDevice() instanceof AudioMixerMediaDevice;
	 * AudioMixerMediaDevice dev = (AudioMixerMediaDevice)
	 * deviceSession.getDevice();
	 * dev.getAudioMixer().addInDataSource(soundDataSource);
	 * 
	 * ((DataSource) deviceSession.getCaptureDevice()). }
	 */
		...
		new Thread(new Runnable() {
			public void run() {
				...
				runOnUiThread(new Runnable() {
					@Override
					public void run() {
						callState.callDuration = ViewUtil.getTextViewValue(
						...
						if (getVideoFragment() != null) {
							getSupportFragmentManager().beginTransaction()
									.remove(getVideoFragment()).commit();
						}
	...
	private void initMicrophoneView() {
		...
		microphoneButton.setOnLongClickListener(new View.OnLongClickListener() {
			public boolean onLongClick(View view) {
				DialogFragment newFragment = VolumeControlDialog
						.createInputVolCtrlDialog();
				newFragment
						.show(getSupportFragmentManager(), "vol_ctrl_dialog");
				return true;
			}
		});
	}
	...
	public boolean dispatchKeyEvent(KeyEvent event) {
		/**
		 * The call to: setVolumeControlStream(AudioManager.STREAM_VOICE_CALL)
	...
	/**
	 * Called when this <tt>Activity</tt> is paused(hidden). Releases all
	 * listeners and leaves the in call notification if the call is in progress.
	 */
	@Override
	...
	private void leaveNotification() {
		...
		CallNotificationManager.get()
				.showCallNotification(this, callIdentifier);
	}
	...
	public void setPeerName(final String name) {
		...
		runOnUiThread(new Runnable() {
			public void run() {
				ActionBarUtil.setTitle(VideoCallActivity.this, getResources()
	...
	void ensureAutoHideFragmentAttached() {
		...
		getSupportFragmentManager().beginTransaction()
				.add(autoHide, AUTO_HIDE_TAG).commit();
	}
	...
	public void ensureAutoHideFragmentDetached() {
		if (autoHide != null) {
			...
			getSupportFragmentManager().beginTransaction().remove(autoHide)
					.commit();
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.call_record_start:
			...
			if (!isRecordingCall) {
				...
				CallManager.startRecorder(call, mFileName, "3gp");

				if (CallManager.isRecLoaded()) {
					isRecordingCall = !isRecordingCall;
					//Toast.makeText(this, "Recorder On", Toast.LENGTH_SHORT)
					//		.show();
				} else {

					//Toast.makeText(this, "Recorder null", Toast.LENGTH_SHORT)
					//		.show();
				}
				/*
				 * try {
				 ...
				 * Iterator<? extends CallPeer> peers = call.getCallPeers(); if
				 * (peers.hasNext()) { CallPeer callPeer = peers.next();
				 * ProtocolProviderService pps = callPeer
				 * .getProtocolProvider();
				 * 
				 * Toast.makeText(this, "B", Toast.LENGTH_SHORT).show(); if (pps
				 * != null) { OperationSetBasicTelephony<?> osvt = pps
				 * .getOperationSet(OperationSetBasicTelephony.class);
				 * 
				 * Toast.makeText(this, "C", Toast.LENGTH_SHORT).show(); if
				 * (osvt != null) rec = osvt.createRecorder(call); else
				 * Toast.makeText(this, "osvt null", Toast.LENGTH_SHORT).show();
				 * } else { Toast.makeText(this, "peer null",
				 * Toast.LENGTH_SHORT).show(); } } else { Toast.makeText(this,
				 * "There aren't any peers in the call",
				 * Toast.LENGTH_SHORT).show();
				 * logger.error("There aren't any peers in the call"); } /
				 * 
				 * rec = telephony.createRecorder(call); if (rec != null) { try
				 * { String currentDateTimeString = new SimpleDateFormat(
				 * "dd-MM-yyyy hh-mm-ss").format(new Date()); String mFileName =
				 * Environment .getExternalStorageDirectory()
				 * .getAbsolutePath(); mFileName += "/" + currentDateTimeString
				 * + ".3gp";
				 * 
				 * rec.start("3gp", mFileName); isRecordingCall =
				 * !isRecordingCall; Toast.makeText(this, "Recorder On",
				 * Toast.LENGTH_SHORT).show(); } catch (IOException e) {
				 * Toast.makeText(this, "Recorder Error " + e.getMessage(),
				 * Toast.LENGTH_SHORT).show(); Log.e("Recorder",
				 * e.getMessage()); } catch (MediaException e) {
				 * Toast.makeText(this, "Recorder Error " + e.getMessage(),
				 * Toast.LENGTH_SHORT).show(); Log.e("Recorder",
				 * e.getMessage()); } } else { Toast.makeText(this,
				 * "Recorder not loaded", Toast.LENGTH_SHORT).show(); } } catch
				 * (Exception e) { Toast.makeText(this, "Recorder Error " +
				 * e.getMessage(), Toast.LENGTH_SHORT).show(); Log.e("Recorder",
				 * e.getMessage()); }
				 */
			} else {
				CallManager.stopRecorder();
				isRecordingCall = !isRecordingCall;
				Toast.makeText(this, "Recorder Off", Toast.LENGTH_SHORT).show();
			}
	...
	private void showZrtpInfoDialog() {
		ZrtpInfoDialog zrtpInfo = ZrtpInfoDialog.newInstance(getIntent()
				.getStringExtra(CallManager.CALL_IDENTIFIER));
	...
	public void propertyChange(PropertyChangeEvent evt) {
		/*
		 * If a Call is added to or removed from the CallConference depicted by
		 * this CallPanel, an update of the view from its model will most likely
		 * be required.
		 */
	...
	public void callPeerRemoved(CallPeerEvent evt) {
		...
		setPeerState(callPeer.getState(), callPeer.getState(), callPeer
				.getState().getLocalizedStateString());
	 ...
	 * by this <tt>CallPanel</tt>, the <tt>Call</tt>s participating in it, the
	 * <tt>CallPeer</tt>s associated with them, the <tt>ConferenceMember</tt>s
	 * participating in any telephony conferences organized by them, etc. In
	 * other words, notifies this instance about any change which may cause an
	 * update to be required so that this view i.e. <tt>CallPanel</tt> depicts
	 * the current state of its model i.e. {@link #callConference}.
	 * 
	 * @param ev
	 *            the <tt>EventObject</tt> this instance is being notified
	 *            about.
	 */
	private void onCallConferenceEventObject(EventObject ev) {
		...
		try {
			...
			if (ev instanceof CallPeerEvent) {
				tryStopCallTimer = (CallPeerEvent.CALL_PEER_REMOVED == ((CallPeerEvent) ev)
						.getEventID());
			} else if (ev instanceof PropertyChangeEvent) {
				PropertyChangeEvent pcev = (PropertyChangeEvent) ev;

				tryStopCallTimer = (CallConference.CALLS.equals(pcev)
						&& (pcev.getOldValue() instanceof Call) && (pcev
						.getNewValue() == null));
			}

			if (tryStopCallTimer
					&& (callConference.isEnded() || callConference
							.getCallPeerCount() == 0)) {
				stopCallTimer();
				doFinishActivity();
			}
	...
	private CallTimerFragment getCallTimerFragment() {
		return (CallTimerFragment) getSupportFragmentManager()
	...
	private void addCallPeerUI(CallPeer callPeer) {
		...
		setPeerState(null, callPeer.getState(), callPeer.getState()
	...
	private void initSecurityStatus() {
		...
		ViewUtil.setTextViewValue(findViewById(android.R.id.content),
				R.id.security_protocol, zrtpCtrl != null ? "zrtp" : "");
	...
	private void setPadlockSecure(boolean isSecure) {
		ViewUtil.setImageViewIcon(findViewById(android.R.id.content),
				R.id.security_padlock, isSecure ? R.drawable.secure_on
						: R.drawable.secure_off);
	}
	...
	public void securityOn(final CallPeerSecurityOnEvent evt) {
		runOnUiThread(new Runnable() {
			public void run() {
				...
				boolean isVerified = zrtpControl != null
				...
				// Protocol name label
				ViewUtil.setTextViewValue(findViewById(android.R.id.content),
						R.id.security_protocol, zrtpControl != null ? "zrtp"
								: "sdes");
	 ...
	 * 
	 * @param parent
	 *            the parent <tt>Context</tt> that will be used to start new
	 *            <tt>Activity</tt>.
