...
/**
 * Dialog fragment displaying technical call information. To create dialog
 * instance factory method {@link #newInstance(String)} should be used. As an
 * argument it takes the call key that identifies a call in {@link CallManager}.
 * 
 * @author Pawel Domas
 */
public class CallInfoDialogFragment extends OSGiDialogFragment {
	...
	/**
	 * Reference to the thread that calculates media statistics and updates the
	 * view.
	 */
	 ...
	 * 
	 * @param callKey
	 *            the key string that identifies active call in
	 *            {@link CallManager}.
	 * 
	 ...
	 * @param isVisible
	 *            flag telling whether the <tt>View</tt> has to be shown or
	 *            hidden.
	 */
	...
	private void doUpdateView() {
		...
		// Identity.
		setTextViewValue(R.id.identity, aCall.getProtocolProvider()
				.getAccountID().getDisplayName());
		// Peer count.
		setTextViewValue(R.id.peerCount,
				String.valueOf(conference.getCallPeerCount()));
		// Conference focus.
		setTextViewValue(R.id.conferenceFocus,
				String.valueOf(conference.isConferenceFocus()));
		// Preferred transport.
		TransportProtocol preferredTransport = aCall.getProtocolProvider()
	 ...
	 * 
	 * @param callPeer
	 *            the <tt>CallPeer</tt>, for which we'll construct the info.
	 */
	private void constructPeerInfo(CallPeer callPeer) {
		...
		String durationStr = GuiUtils.formatTime(startTime.getTime(),
				System.currentTimeMillis());
		setTextViewValue(R.id.callDuration, durationStr);

		CallPeerMediaHandler<?> callPeerMediaHandler = null;
		if (callPeer instanceof MediaAwareCallPeer) {
			callPeerMediaHandler = ((MediaAwareCallPeer<?, ?, ?>) callPeer)
					.getMediaHandler();
		}
		// Audio stream info.
	 ...
	 *            the call peer for which ICE information will be displayed.
	 */
	...
	private void updateIceSection(CallPeerMediaHandler<?> callPeerMediaHandler) {
		...
		boolean iceStateVisible = iceState != null
				&& !iceState.equals("Terminated");
		...
		if (iceStateVisible) {
			...
			int strId = resources.getIdentifier(
					"service_gui_callinfo_ICE_STATE_" + iceState.toUpperCase(),
		...
		if (isTotalHarvestTime) {
			String harvestStr = getHarvestTimeStr(harvestingTime,
		...
		String[] harvesterNames = { "GoogleTurnCandidateHarvester",
				"GoogleTurnSSLCandidateHarvester", "HostCandidateHarvester",
				"JingleNodesHarvester", "StunCandidateHarvester",
				"TurnCandidateHarvester", "UPNPHarvester" };
		int[] harvesterLabels = { R.id.googleTurnLabel,
				R.id.googleTurnSSlLabel, R.id.hostHarvesterLabel,
				R.id.jingleNodesLabel, R.id.stunHarvesterLabel,
				R.id.turnHarvesterLabel, R.id.upnpHarvesterLabel };
		int[] harvesterValues = { R.id.googleTurnTime, R.id.googleTurnSSlTime,
				R.id.hostHarvesterTime, R.id.jingleNodesTime,
				R.id.stunHarvesterTime, R.id.turnHarvesterTime,
				R.id.upnpHarvesterTime };
		for (int i = 0; i < harvesterLabels.length; ++i) {
			harvestingTime = 0;

			if (callPeerMediaHandler != null) {
				harvestingTime = callPeerMediaHandler
						.getHarvestingTime(harvesterNames[i]);
			}
			...
			if (visible) {
				int harvestCount = callPeerMediaHandler
						.getNbHarvesting(harvesterNames[i]);

				setTextViewValue(getView(), harvesterValues[i],
						getHarvestTimeStr(harvestingTime, harvestCount));
			}
		}
	}
	...
	private String getHarvestTimeStr(long harvestingTime, int harvestCount) {
		...
		harvestStr.append(" ");
		harvestStr
				.append(getString(R.string.service_gui_callinfo_HARVESTING_MS_FOR));
		harvestStr.append(" ");
		harvestStr.append(harvestCount);
		harvestStr.append(getString(R.string.service_gui_callinfo_HARVESTS));

		return harvestStr.toString();
	}

	/**
	 * Creates the string for the stream encryption method (null, MIKEY, SDES,
	 * ZRTP) used for a given media stream (type AUDIO or VIDEO).
	 * 
	 * @param callPeerMediaHandler
	 *            The media handler containing the different media streams.
	 * @param mediaStream
	 *            the <tt>MediaStream</tt> that gives us access to audio/video
	 *            info.
	 * @param mediaType
	 *            The media type used to determine which stream of the media
	 *            handler must returns it encryption method.
	 */
	private String getStreamEncryptionMethod(
			CallPeerMediaHandler<?> callPeerMediaHandler,
			MediaStream mediaStream, MediaType mediaType) {
		Resources resources = getResources();

		String transportProtocolString = "";
		StreamConnector.Protocol transportProtocol = mediaStream
				.getTransportProtocol();
		...
		if (srtpControl != null) {
			...
			rtpType = resources
					.getString(R.string.service_gui_callinfo_MEDIA_STREAM_SRTP)
					+ " ("
					+ resources
							.getString(R.string.service_gui_callinfo_KEY_EXCHANGE_PROTOCOL)
					+ ": " + info + ")";
		}
		// If the stream is not secured.
		else {
			rtpType = resources
					.getString(R.string.service_gui_callinfo_MEDIA_STREAM_RTP);
		}
	 ...
	 * 
	 * @param callPeerMediaHandler
	 *            The <tt>CallPeerMadiaHandler</tt> containing the AUDIO/VIDEO
	 *            stream.
	 * @param mediaType
	 *            The media type used to determine which stream of the media
	...
	private void updateAudioVideoInfo(
			CallPeerMediaHandler<?> callPeerMediaHandler, MediaType mediaType) {
		View container = mediaType == MediaType.AUDIO ? getView().findViewById(
				R.id.audioInfo) : getView().findViewById(R.id.videoInfo);
		...
		setTextViewValue(
				container,
				R.id.mediaTransport,
				getStreamEncryptionMethod(callPeerMediaHandler, mediaStream,
						mediaType));
		// Set the title label to Video info if it's a video stream.
		if (mediaType == MediaType.VIDEO) {
			setTextViewValue(container, R.id.audioVideoLabel,
					getString(R.string.service_gui_callinfo_VIDEO_INFO));
		}

		boolean hasVideoSize = false;

		if (mediaType == MediaType.VIDEO) {
			Dimension downloadVideoSize = mediaStreamStats
		...
		// Codec.
		setTextViewValue(container, R.id.codec, mediaStreamStats.getEncoding()
				+ " / " + mediaStreamStats.getEncodingClockRate() + " Hz");

		boolean displayedIpPort = false;

		// ICE candidate type.
		String iceCandidateExtendedType = callPeerMediaHandler
				.getICECandidateExtendedType(mediaType.toString());
		...
		if (iceCandidateExtVisible) {
			setTextViewValue(container, R.id.iceExtType,
					iceCandidateExtendedType);
			displayedIpPort = true;
		}

		// Local host address.
		InetSocketAddress iceLocalHostAddress = callPeerMediaHandler
				.getICELocalHostAddress(mediaType.toString());
		...
		if (iceLocalHostVisible) {
			setTextViewValue(container, R.id.iceLocalHost,
		...
		InetSocketAddress iceLocalReflexiveAddress = callPeerMediaHandler
				.getICELocalReflexiveAddress(mediaType.toString());
		...
		if (iceLocalReflexiveVisible) {
			setTextViewValue(container, R.id.iceLocalReflx,
					iceLocalReflexiveAddress.getAddress().getHostAddress()
							+ "/" + iceLocalReflexiveAddress.getPort());
			displayedIpPort = true;
		}

		// Local relayed address.
		InetSocketAddress iceLocalRelayedAddress = callPeerMediaHandler
				.getICELocalRelayedAddress(mediaType.toString());
		...
		ensureVisible(container, R.id.iceLocalRelayedLabel,
				iceLocalRelayedVisible);

		if (iceLocalRelayedAddress != null) {
			setTextViewValue(container, R.id.iceLocalRelayed,
					iceLocalRelayedAddress.getAddress().getHostAddress() + "/"
							+ iceLocalRelayedAddress.getPort());
			displayedIpPort = true;
		}

		// Remote relayed address.
		InetSocketAddress iceRemoteRelayedAddress = callPeerMediaHandler
				.getICERemoteRelayedAddress(mediaType.toString());
		...
		if (isIceRemoteRelayed) {
			setTextViewValue(container, R.id.iceRemoteRelayed,
					iceRemoteRelayedAddress.getAddress().getHostAddress() + "/"
							+ iceRemoteRelayedAddress.getPort());
			displayedIpPort = true;
		}

		// Remote reflexive address.
		InetSocketAddress iceRemoteReflexiveAddress = callPeerMediaHandler
				.getICERemoteReflexiveAddress(mediaType.toString());
		...
		if (isIceRemoteReflexive) {
			setTextViewValue(container, R.id.iceRemoteReflexive,
					iceRemoteReflexiveAddress.getAddress().getHostAddress()
							+ "/" + iceRemoteReflexiveAddress.getPort());
			displayedIpPort = true;
		}

		// Remote host address.
		InetSocketAddress iceRemoteHostAddress = callPeerMediaHandler
				.getICERemoteHostAddress(mediaType.toString());
		...
		if (isIceRemoteHost) {
			setTextViewValue(container, R.id.iceRemoteHost,
		...
		// Decoded with FEC.
		setTextViewValue(container, R.id.decodedWithFEC,
		...
		// Discarded total.
		String discardedTotalStr = String.valueOf(mediaStreamStats
				.getNbDiscarded())
				+ " ("
				+ mediaStreamStats.getNbDiscardedLate()
				+ " late, "
				+ mediaStreamStats.getNbDiscardedFull()
				+ " full, "
				+ mediaStreamStats.getNbDiscardedShrink()
				+ " shrink, "
				+ mediaStreamStats.getNbDiscardedReset() + " reset)";
		setTextViewValue(container, R.id.discardedTotal, discardedTotalStr);

		// Adaptive jitter buffer.
		setTextViewValue(container, R.id.adaptiveJitterBuffer,
				mediaStreamStats.isAdaptiveBufferEnabled() ? "enabled"
						: "disabled");

		// Jitter buffer delay.
		String jitterDelayStr = "~" + mediaStreamStats.getJitterBufferDelayMs()
		...
		// Jitter.
		setTextViewValue(container, R.id.jitter, DOWN_ARROW + " "
				+ (int) mediaStreamStats.getDownloadJitterMs() + " ms "
				+ UP_ARROW + (int) mediaStreamStats.getUploadJitterMs() + " ms");
	}
	 ...
	 * 
	 * @param videoSize
	 *            The video size Dimension, containing the width and the height
	 *            of the video.
	 * 
	...
	private String videoSizeToString(Dimension videoSize) {
		...
		return ((int) videoSize.getWidth()) + " x "
	...
	private void updateMediaStats() {
		...
		for (CallPeer callPeer : conference.getCallPeers()) {
			...
			CallPeerMediaHandler<?> callPeerMediaHandler = ((MediaAwareCallPeer<?, ?, ?>) callPeer)
					.getMediaHandler();
			...
			calcStreamMediaStats(callPeerMediaHandler
	 ...
	 * 
	 * @param mediaStream
	 *            the media stream that will have it's statistics recalculated.
	 */
	...
	/**
	 * The thread that periodically recalculates media stream statistics and
