...
/**
 * The <tt>AccountLoginActivity</tt> is the activity responsible for creating a
 * new account.
 * 
 * @author Yana Stamcheva
 * @author Pawel Domas
 */
public class AccountLoginActivity extends ExitMenuActivity implements
		AccountLoginFragment.AccountLoginListener {
	/**
	 ...
	 *         signed in account
	 */
	private ProtocolProviderService signIn(String userName, String password,
			String protocolName) {
		...
		for (int i = 0; i < accountWizardRefs.length; i++) {
			AccountRegistrationWizard accReg = (AccountRegistrationWizard) bundleContext
		...
		try {
			...
			if (e.getErrorCode() == OperationFailedException.ILLEGAL_ARGUMENT) {
				AndroidUtils.showAlertDialog(this,
			...
			AndroidUtils.showAlertDialog(this, R.string.service_gui_ERROR,
					R.string.service_gui_ACCOUNT_CREATION_FAILED);
		}
		return null;
	}
	...
	public void onLoginPerformed(String login, String password, String network) {
		ProtocolProviderService protocolProvider = signIn(login, password,
				network);
