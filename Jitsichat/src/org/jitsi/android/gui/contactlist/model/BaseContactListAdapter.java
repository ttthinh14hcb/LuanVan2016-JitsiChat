...
public abstract class BaseContactListAdapter extends BaseExpandableListAdapter {
	...
	private final Logger logger = Logger
			.getLogger(BaseContactListAdapter.class);
	...
	protected void updateDisplayName(final int groupIndex,
		...
		View contactView = contactListView.getChildAt(getListIndex(groupIndex,
				contactIndex) - firstIndex);

		if (contactView != null) {
			MetaContact metaContact = (MetaContact) getChild(groupIndex,
					contactIndex);

			ViewUtil.setTextViewValue(contactView, R.id.displayName,
	...
	protected void updateAvatar(final int groupIndex, final int contactIndex,
		...
		if (contactView != null) {
			ImageView avatarView = (ImageView) contactView
					.findViewById(R.id.avatarIcon);

			if (avatarView != null)
				setAvatar(avatarView, getContactRenderer(groupIndex)
	 ...
	 *            contact implementation object instance
	 */
	protected void updateStatus(final int groupIndex, final int contactIndex,
			Object contactImpl) {
		int firstIndex = contactListView.getFirstVisiblePosition();
		...
		if (contactView != null) {
			ImageView statusView = (ImageView) contactView
			...
			statusView.setImageDrawable(getContactRenderer(groupIndex)
	...
	public int getListIndex(int groupIndex, int contactIndex) {
		...
		for (int i = 0; i <= lastIndex; i++) {
			long longposition = contactListView.getExpandableListPosition(i);
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		if (convertView == null) {
			LayoutInflater inflater = contactListFragment.getActivity()
					.getLayoutInflater();
			convertView = inflater.inflate(R.layout.contact_list_row, parent,
					false);

			if (isExtendedChat) {
				// Show shadows on the right side for tablet layout
				ViewUtil.ensureVisible(convertView, R.id.rightShadowTop, true);
				ViewUtil.ensureVisible(convertView, R.id.rightShadowBottom,
						true);
			}

			contactViewHolder = new ContactViewHolder();
			contactViewHolder.displayName = (TextView) convertView
					.findViewById(R.id.displayName);
			contactViewHolder.statusMessage = (TextView) convertView
					.findViewById(R.id.statusMessage);

			contactViewHolder.avatarView = (ImageView) convertView
					.findViewById(R.id.avatarIcon);
			...
			contactViewHolder.statusView = (ImageView) convertView
					.findViewById(R.id.contactStatusIcon);
			contactViewHolder.callButton = (ImageView) convertView
					.findViewById(R.id.contactCallButton);
			contactViewHolder.selectedBgView = (ImageView) convertView
					.findViewById(R.id.selectedBackgroundIcon);
			contactViewHolder.buttonSeparatorView = (ImageView) convertView
					.findViewById(R.id.buttonSeparatorView);

			contactViewHolder.callButtonLayout = convertView
					.findViewById(R.id.callButtonLayout);
			...
			contactViewHolder.callButtonLayout.setTag(contactViewHolder);
		...
		if (renderer.isSelected(child)) {
			convertView
					.setBackgroundResource(R.drawable.list_selection_gradient);

			if (isExtendedChat) {
				contactViewHolder.selectedBgView.setVisibility(View.VISIBLE);
			}
		} else {
			convertView.setBackgroundResource(R.drawable.contact_list_selector);

			if (isExtendedChat) {
				contactViewHolder.selectedBgView.setVisibility(View.GONE);
			}
		}

		// Set display name value.
		contactViewHolder.displayName.setText(renderer.getDisplayName(child));
		...
		contactViewHolder.statusView.setImageDrawable(renderer
				.getStatusImage(child));
		...
		if (isShowVideoCall || isShowCall) {
			...
			AndroidUtils
					.setOnTouchBackgroundEffect(contactViewHolder.callButtonLayout);

			if (renderer.isSelected(child)) {
				if (isShowVideoCall)
					contactViewHolder.callButton
							.setImageResource(R.drawable.video_call_selected);
				else
					contactViewHolder.callButton
							.setImageResource(R.drawable.contact_call_selected);
			} else {
				if (isShowVideoCall)
					contactViewHolder.callButton
							.setImageResource(R.drawable.video_call);
				else
					contactViewHolder.callButton
							.setImageResource(R.drawable.contact_call);
			}
		} else {
	...
	public View getGroupView(int groupPosition, boolean isExpanded,
		...
		if (convertView == null) {
			...
			groupViewHolder.displayName = (TextView) convertView
					.findViewById(R.id.displayName);
			groupViewHolder.indicator = (ImageView) convertView
					.findViewById(R.id.groupIndicatorView);
		...
		int indicatorResId = isExpanded ? R.drawable.expanded
	...
	private class CallButtonClickListener implements View.OnClickListener {
		public void onClick(View view) {
			...
			Object contact = getChild(viewHolder.groupPosition,
			 ...
			 * Thinh
			 */
			String contactAddress = getContactRenderer(viewHolder.groupPosition)
					.getDefaultAddress(contact);

			boolean isPhone = false;
			if (!contactAddress.contains("@")) {
				try {
					String xx = contactAddress.replace(" ", "");
					int number = Integer.parseInt(xx);

					Intent callIntent = new Intent(Intent.ACTION_CALL);
					callIntent.setData(Uri.parse("tel:" + String.valueOf(number)));

					contactListFragment.getActivity().startActivity(callIntent);
					isPhone = true;
	...
	private class AvatarIconClickListener implements View.OnClickListener {
		public void onClick(View view) {
			...
			String contactAddress = getContactRenderer(groupPos)
					.getDefaultAddress(contact);
	...
	private void setAvatar(ImageView avatarView, Drawable avatarImage) {
		if (avatarImage == null) {
			avatarImage = JitsiApplication.getAppResources().getDrawable(
					R.drawable.avatar);
		}
