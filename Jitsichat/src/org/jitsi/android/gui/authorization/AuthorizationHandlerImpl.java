...
public class AuthorizationHandlerImpl implements AuthorizationHandler {
	...
	private static Map<Long, AuthorizationRequestedHolder> requestMap = new HashMap<Long, AuthorizationRequestedHolder>();
	...
	public AuthorizationResponse processAuthorisationRequest(
		...
		AuthorizationRequestedHolder requestHolder = new AuthorizationRequestedHolder(
				id, req, sourceContact);
	 ...
	 * method.
	 * <p>
	 * The method will be called any whenever someone acts upone an
	 * authorization request that we have previously sent.
	 */
	@Override
	public void processAuthorizationResponse(AuthorizationResponse response,
		...
		AuthorizationResponse.AuthorizationResponseCode responseCode = response
				.getResponseCode();

		if (responseCode == AuthorizationResponse.ACCEPT) {
			msg = contact.getDisplayName()
					+ " "
					+ ctx.getString(R.string.service_gui_AUTHORIZATION_ACCEPTED);
		} else if (responseCode == AuthorizationResponse.REJECT) {
			msg = contact.getDisplayName()
					+ " "
					+ ctx.getString(R.string.service_gui_AUTHENTICATION_REJECTED);
		}
	...
	static public class AuthorizationRequestedHolder {
		 ...
		 *            contact related to the request
		 */
		public AuthorizationRequestedHolder(Long ID,
		...
		/**
		 * Method should be used by the dialog activity to notify about the
		 * result.
		 * 
		 * @param response
		 */
		 ...
		 * 
		 * @param requestText
		 *            the text that will be added to the authorization request.
		 */
