...
public class ChatFragment extends OSGiFragment {
	...
	/**
	 * Indicates that this fragment is visible to the user. This is important,
	 * because of PagerAdapter being used on phone layouts, which doesn't
	 * properly call onResume() when switched page fragment is displayed.
	 */
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		View content = inflater.inflate(R.layout.chat_conversation, container,
				false);
		...
		this.header = inflater.inflate(R.layout.progressbar, chatListView,
				false);
		...
		chatListView.setOnScrollListener(new AbsListView.OnScrollListener() {
			...
			public void onScroll(AbsListView view, int firstVisibleItem,
				...
				scrollTopOffset = firstVisible != null ? firstVisible.getTop()
						: 0;
			}
		});
	 ...
	 * 
	 * @param isVisible
	 *            <tt>true</tt> if the fragment is now visible to the user.
	 * @see ChatController
	 */
	...
	private void checkInitController() {
		if (visibleToUser && chatListView != null) {
			...
			AndroidGUIActivator.getLoginRenderer().addGlobalStatusListener(
			...
			AndroidGUIActivator.getLoginRenderer().removeGlobalStatusListener(
					globalStatusListener);
		} else {
	...
	private void initAdapter() {
		/**
		 * Initial history load is delayed until the chat is displayed to the
		 * user. We previously relyed on onCreate, but it will be called too
		 * early on phone layouts where ChatPagerAdapter is used. It creates
	...
	public void onPause() {
		...
		/*
		 * Indicates that this fragment is no longer visible, because of this
		 * call parent <tt>Activities don't have to call it in onPause().
		 */
	...
	class ChatListAdapter extends BaseAdapter implements
			ChatSession.ChatSessionListener, ContactPresenceStatusListener,
			TypingNotificationsListener {
		/**
		 * The list of chat message displays. All access and modification of
		 * this list must be done on the UI thread.
		 */
		private final List<MessageDisplay> messages = new ArrayList<MessageDisplay>();
		...
		/**
		 * Passes the message to the <tt>ChatListAdapter</tt> for processing and
		 * appends it at the end.
		 * 
		 * @param newMessage
		 *            the message to add.
		 * @param update
		 *            if set to <tt>true</tt> will notify the UI about update
		 *            immediately.
		 */
		public void addMessage(final ChatMessage newMessage,
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					if (chatListAdapter == null) {
						logger.warn("Add message handled, when there's no adapter"
		...
		private void addMessageImpl(ChatMessage newMessage, boolean update) {
			...
			ChatMessage lastMsg = lastMsgIdx != -1 ? chatListAdapter
					.getMessage(lastMsgIdx) : null;
			...
			if (lastMsg == null || !lastMsg.isConsecutiveMessage(newMessage)) {
				...
				messages.get(lastMsgIdx).update(
			...
			if (update) {
				...
				chatListView.setSelection(msgIdx
						+ chatListView.getHeaderViewsCount());
			}
		}

		/**
		 * Inserts given <tt>Collection</tt> of <tt>ChatMessage</tt> at the
		 * beginning of the list.
		 * 
		 * @param collection
		 *            the collection of <tt>ChatMessage</tt> to prepend.
		 */
		public void prependMessages(Collection<ChatMessage> collection) {
			...
			while (iterator.hasNext()) {
				...
				if (previous == null
						|| !previous.msg.isConsecutiveMessage(next)) {
		 ...
		 * <tt>newMessage</tt> is a correction message, then the last message of
		 ...
		 *         merging process. If <tt>newMessage</tt> is a correction
		 *         message, then the last message of the same type will be
		 *         returned.
		 */
		...
		public int getItemViewType(int position) {
			...
			if (messageType == ChatMessage.INCOMING_MESSAGE)
				...
				/*
				 * logger.error( "pos: " + position + "S UID: " + sessionCorrUID
				 * + "M UID: " + msgCorrUID );
				 */
		...
		private final View.OnClickListener msgClickAdapter = new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (chatController != null && v.getTag() instanceof Integer) {
					...
					chatController
							.onItemClick(chatListView, v, position, -1/*
																	 * id not
																	 * used
																	 */);
				}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (convertView == null) {
				...
				convertView = inflateViewForType(viewType, messageViewHolder,
						parent);
			} else {
				...
				if (messageViewHolder.viewType == CORRECTED_MESSAGE_VIEW
					...
					if (messageViewHolder.viewType != currentViewType) {
						...
						convertView = inflateViewForType(currentViewType,
		...
		private View inflateViewForType(int viewType,
				MessageViewHolder messageViewHolder, ViewGroup parent) {
			...
			if (viewType == INCOMING_MESSAGE_VIEW) {
				...
				messageViewHolder.avatarView = (ImageView) convertView
						.findViewById(R.id.incomingAvatarIcon);

				messageViewHolder.statusView = (ImageView) convertView
						.findViewById(R.id.incomingStatusIcon);

				messageViewHolder.messageView = (TextView) convertView
						.findViewById(R.id.incomingMessageView);

				messageViewHolder.timeView = (TextView) convertView
						.findViewById(R.id.incomingTimeView);

				messageViewHolder.typingView = (ImageView) convertView
				...
				if (viewType == OUTGOING_MESSAGE_VIEW) {
					...
					convertView = inflater.inflate(R.layout.chat_corrected_row,
							parent, false);
				}

				messageViewHolder.avatarView = (ImageView) convertView
						.findViewById(R.id.outgoingAvatarIcon);

				messageViewHolder.statusView = (ImageView) convertView
						.findViewById(R.id.outgoingStatusIcon);

				messageViewHolder.messageView = (TextView) convertView
						.findViewById(R.id.outgoingMessageView);

				messageViewHolder.timeView = (TextView) convertView
						.findViewById(R.id.outgoingTimeView);

				messageViewHolder.outgoingMessageHolder = convertView
						.findViewById(R.id.outgoingMessageHolder);
			} else {
				// System or error view
				convertView = inflater
						.inflate(
								viewType == SYSTEM_MESSAGE_VIEW ? R.layout.chat_system_row
										: R.layout.chat_error_row, parent,
								false);
			...
			messageViewHolder.messageView.setMovementMethod(LinkMovementMethod
					.getInstance());
			// Set clicks adapter
		 ...
		 * 
		 * @param viewHolder
		 *            the <tt>MessageViewHolder</tt> to update.
		 */
		private void updateStatusAndAvatarView(MessageViewHolder viewHolder) {
			Drawable avatar;
			Drawable status;
			if (viewHolder.viewType == INCOMING_MESSAGE_VIEW) {
				avatar = MetaContactRenderer.getAvatarDrawable(chatSession
						.getMetaContact());

				status = MetaContactRenderer.getStatusDrawable(chatSession
						.getMetaContact());
			} else if (viewHolder.viewType == OUTGOING_MESSAGE_VIEW
					|| viewHolder.viewType == CORRECTED_MESSAGE_VIEW) {
				AndroidLoginRenderer loginRenderer = AndroidGUIActivator
						.getLoginRenderer();
		...
		public void messageReceived(final MessageReceivedEvent evt) {
			...
			final MetaContact metaContact = AndroidGUIActivator
					.getContactListService().findMetaContactByContact(
							protocolContact);
		...
		public void typingNotificationReceived(TypingNotificationEvent evt) {
			...
			TypingNotificationHandler.handleTypingNotificationReceived(evt,
					ChatFragment.this);
		}
			...
			runOnUiThread(new Runnable() {
				...
				public void run() {
					for (int i = 0; i < chatListView.getChildCount(); i++) {
						...
						MessageViewHolder viewHolder = (MessageViewHolder) row
								.getTag();
						if (viewHolder == null)
							continue;
		...
		class MessageDisplay {
			 ...
			 * 
			 * @param msg
			 *            the <tt>ChatMessage</tt> that will be displayed by
			 *            this instance.
			 */
	 ...
	 */
	private class LoadHistoryTask extends
			AsyncTask<Void, Void, Collection<ChatMessage>> {
		/**
	...
	private class UpdateStatusTask extends AsyncTask<Void, Void, Void> {
		...
		protected void onPostExecute(Void result) {
			...
			for (int i = 0; i < chatListView.getChildCount(); i++) {
				...
				MessageViewHolder viewHolder = (MessageViewHolder) chatRowView
						.getTag();

				if (viewHolder != null
						&& viewHolder.viewType == ChatListAdapter.INCOMING_MESSAGE_VIEW) {
					Drawable status = MetaContactRenderer
							.getStatusDrawable(chatSession.getMetaContact());
	 ...
	 * @param avatarDrawable
	 *            the avatar drawable to set
	 */
	public void setAvatar(ImageView avatarView, Drawable avatarDrawable) {
		if (avatarDrawable == null) {
			avatarDrawable = JitsiApplication.getAppResources().getDrawable(
					R.drawable.avatar);
		}
	 ...
	 * 
	 * @param typingState
	 *            the typing state that should be represented in the view
	 */
	public void setTypingState(int typingState) {
		if (typingView == null)
			return;

		TextView typingTextView = (TextView) typingView
				.findViewById(R.id.typingTextView);
		ImageView typingImgView = (ImageView) typingView
				.findViewById(R.id.typingImageView);
		...
		if (typingState == OperationSetTypingNotifications.STATE_TYPING) {
			...
			typingTextView.setText(chatSession.getShortDisplayName()
					+ " "
					+ JitsiApplication
							.getResString(R.string.service_gui_CONTACT_TYPING));
			setVisible = true;
		} else if (typingState == OperationSetTypingNotifications.STATE_PAUSED) {
			typingImgView.setImageResource(R.drawable.typing1);
			typingTextView
					.setText(chatSession.getShortDisplayName()
							+ " "
							+ JitsiApplication
									.getResString(R.string.service_gui_CONTACT_PAUSED_TYPING));
			setVisible = true;
		}
